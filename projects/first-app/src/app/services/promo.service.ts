...
export class PromoService implements OnInit {
  ...
  /**
 * Traite une promotion utilisée
 * Si la promotion est réutilisable, elle n'est pas supprimée
 */
usePromo(id: string): void {
    const promo = this.promos.find(p => p.id === id);
    
    if (promo) {
      // Si la promotion n'est pas réutilisable, la supprimer
        // Désélectionner la promotion même si elle est réutilisable
